...
class MusicBot(discord.Client):
    ...
    async def cmd_help(self,message, command=None):
        ...
        Prints a help message. If a command is specified, it prints a help message for that command.
        Otherwise, it lists the available commands.
        """

        if command:
            cmd = getattr(self, 'cmd_' +command.lower(), None)
            embed=discord.Embed(title=str(command)+" Help", color=0xe0e0e0)
            embed.set_thumbnail(url="https://forums.androidcentral.com/images/forum_icons/1126.png")
            if bool(cmd):
                desc="{}".format(dedent(str(cmd.__doc__)),command_prefix=self.config.command_prefix)
                embed.add_field(name="", value=desc, inline=True)
            else:
                embed.add_field(name="", value="No such command", inline=True)  
            embed.set_footer(text="Contact Black Arrow for further assistance")
            await self.send_message(message.channel,embed=embed)
        else:
            embed=discord.Embed(title="Commands List", color=0xe0e0e0)
            embed.set_thumbnail(url="https://forums.androidcentral.com/images/forum_icons/1126.png")
            #helpmsg = "**Commands**\n```"
            #commands = []

            for att in dir(self):
                if att.startswith('cmd_') and att in ['cmd_play','cmd_queue','cmd_np','cmd_skip','cmd_search','cmd_clean','cmd_join','cmd_leave','cmd_pause','cmd_resume','cmd_volume','cmd_lyrics','cmd_repeat','cmd_promote','cmd_remove','cmd_playnow','cmd_credit']:
                    command_name = att.replace('cmd_', '').lower()
                    comma=getattr(self, att.lower(), None)
                    desc="{}".format(dedent(str(comma.__doc__)),command_prefix=self.config.command_prefix)
                    embed.add_field(name=command_name, value=str(desc), inline=True)
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        """
        Usage: play song_link || play name of song to search for
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        """
        Usage: search [service] [number_of_returned_query] song_name

        Searches mentioned below services for a song and waits for user to choose one before adds it to the queue.
        """
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        Displays the current song in text channel.
        """
    ...
    async def cmd_join(self, channel, author, voice_channel):
        ...
        Request the bot to the user's voice channel.
        """
    ...
    async def cmd_shuffle(self, channel, player):
        ...
        Shuffles the playlist to avoid bordem and monotony.
        """
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        Skips the current song when enough votes are for.
        """
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        Sets the volume volume from 1 to 100.
        """
    ...
    def searchSong(self, song_name):
        ...
        encodedsong = urllib.parse.quote_plus(song_name)
        print("[Lyrics] Search Url: " + encodedsong)
        page = requests.get('http://lyrics.wikia.com/wiki/Special:Search?query=' + encodedsong)
        tree =html.fromstring(page.content)
        songs = tree.xpath('//li[@class="result"]/article/h1/a/text()')
        return songs
    async def cmd_lyrics (self, channel, player, song_url=None):
        """
        Usage: lyrics || lyrics [song_url]
            Displays the lyrics of the current song or the specified in the URL
        """
        if song_url:
            try:
                info = await self.downloader.extract_info(player.playlist.loop, song_url, download=False, process=False)
                ...
                return Response("cannot find the title, please ensure you specified an URL")
        elif not player.current_entry:
            return Response("No song currently being played, pls specify an url or play a song")
        else:
        ...
        for item in songsResults:
            ...
            try:
                if nameofthesong not in title:
                    print("%s was not the song you were looking for were you?" % nameofthesong)
                    continue
            except:
                pass
            try:
                ...
                print("[Lyrics] failed to get lyrics from" + item)
                try:
                    count = 0
                    artist = item.split(':')[0]
                    ...
                    continue
        return Response("i failed you couldn't get the lyrics i am sorry :cry: ")
    async def cmd_credit (self,channel):
        """
        Usage: credit

        states her owner
        """        
        return Response("my name is Lucina, ßlack Arrow created me ^^" ,delete_after=8)

    async def cmd_hey (self,channel):
        """
        Usage: hey

        she ask a serious question
        """
        quotes=['have you seen my tibber?', 'You wanna play too? It ll be fun!', 'Ashes, ashes, they all fall down <:hehee:393052730329137153>','Come out, come out, wherever you are!','I never play with matches <:fiteme:393053430064742404>']        
        return Response(random.choice(quotes) ,delete_after=3)

    async def cmd_pldump(self, channel, song_url):
        """
        Usage: pldump url

        Dumps urls of a playlist
        """
    ...
    async def cmd_promote(self, player, position=None):
        """
        Usage: promote || promote [song position]

        Promotes the last song in the queue to the front. If you specify a position, it promotes the song at that position to the front.
        """
    ...
    async def cmd_remove(self, player, position=None):
        """
        Usage: remove || remove [song position]

        Removes the next song from the queue. If you specify a position, it removes the song at that position from the queue.
        """
    ...
    async def cmd_playnow(self, player, channel, author, permissions, leftover_args, song_url):
        """
        Usage: playnow [song_link] || playnow song to search for

        Stops the currently playing song and immediately plays the song requested. If a link is not provided, the first result from a youtube search is played.
        """
    ...
    async def cmd_roll(self, channel, author, leftover_args):
        ...
        Usage: roll 
         ...
         roll one dice and show the result.
        """
        #usage [1-100]d[MAXROLL] || roll [MAXROLL] ||Roll the set number of dice and show the sum of the rolls, or Commented for future understanding || 
        if not leftover_args:
            raise exceptions.CommandError("Unable to roll dice. Usage: roll [NUMDICE]d[4-20] or roll [MAXROLL].")
        ...
        if len(diceVals) == 2:
            ...
            if not (1 <= numDice <= 100):
                raise exceptions.CommandError("Unable to roll dice. Usage: roll [1-100]d[MAXROLL].")
            if maxRoll < 1:
